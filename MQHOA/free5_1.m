...
for rep=1:repeat
    ...
    while 1 % 尺度迭代开始
        ...
        %两次的最小值没有发生变化，(因为随机性的存在使得v_min2与v_min几乎没有相等的时候，几乎不会尺度收缩，所以选择二者的差值达到一定小，就认为稳定)认为达到了暂稳态
        ...
        if sigma<=sigmaMin | w>MFE   %达到最大迭代次数或者收敛到足够小
            break;
        end
    end 
    tot_time(rep) = toc; 
...
fprintf('自由粒子，只要有两次满足收敛条件就尺度收缩，不管是不是连续达到的');
fprintf('\n');
...
a=sig;   %reapeat重复试验次数，每一次最后收敛到的尺度
save a;
b=gbestV;
save b;
